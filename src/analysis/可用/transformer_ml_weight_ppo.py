...
def main():
    ...
    if has_cpi and 'cycles_per_instruction' in feats:
        ...
        max_weights[cpi_idx] = 2.5
        print(f"-> CPI 索引 {cpi_idx}: 范围限制为 [0.8, 2.5]")
        ...
        for epoch in range(40):
            ...
            for batch in current_loader:
                ...
                if weights is not None:
                    ...
                    loss_main = alpha * loss_seq + (1 - alpha) * loss_weighted + 1e-4 * z.abs().mean()
                    # ============ 新增：权重熵正则化 (就在这里插入) ============
                    # 目的：防止权重全部挤在 5.0 或 0.1，让权重分布更均匀、更具探索性
                    if epoch >= 10:  # 只在权重解冻后应用
                        # 归一化权重以计算分布熵
                        w_probs = w_use / (w_use.sum() + 1e-6)
                        weight_entropy = -torch.sum(w_probs * torch.log(w_probs + 1e-6))

                        # 我们希望熵越大越好（分布越平均），所以 loss 要减去熵
                        # 系数 0.01 可以根据 5090 的跑数情况微调：如果还是撞墙就调大到 0.05
                        loss_main = loss_main - 0.01 * weight_entropy
                        # ========================================================
