...
def preprocess_all_features(sequences, feats, has_cpi):
    ...
    if has_cpi:
        ...
        cpi_processed = process_cpi_special(cpi_raw, target_mean=0.0, target_std=0.05)
...
# 绘图
    ...
    # ===== Residual PPO Agent（只初始化一次）=====
    residual_ppo = ResidualPPOAgent(
        state_dim=12,
        residual_scale=0.02,
        device=DEVICE
    )
    ...
    for i, sample_data in enumerate(ppo_samples[:1000]):  # 评估前1000个样本，修改变量名
        ...
        # 👉👉👉 【就在这里：构造 state】 👈👈👈
        z_vec = sample_data['z']          # shape: (latent_dim,)
        cpu_dev = sample_data['cpu_dev']
        mem_dev = sample_data['mem_dev']

        state = np.concatenate([
            z_vec[:8],                     # (8,)
            np.array([cpu_dev, mem_dev]),  # (2,)
            np.array([cpu_safe, mem_safe]) # (2,)
        ], axis=0)

        # === Residual PPO ===
        cpu_dir = np.sign(cpu_safe)
        mem_dir = np.sign(mem_safe)

        cpu_res, mem_res = residual_ppo.select_residual(
            state, cpu_dir, mem_dir
        )
