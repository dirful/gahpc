...
def main():
    ...
    criterion_static = nn.CosineEmbeddingLoss() # nn.MSELoss()  # 约束损失：预测静态 vs. 真实静态
    alpha = 0.7
    ...
    for epoch in range(40):
        ...
        for batch in loader:
            ...
            loss_main = alpha * loss_seq + (1 - alpha) * loss_weighted + 1e-4 * z.abs().mean() + 1e-3 * (weight_module.delta ** 2).mean()
            # 新增：约束损失（只引导，不主导距离计算）
            loss_const = 0.0
        ...
        if model_class == WeightedTransAE:
            ...
            class LSTMAE(nn.Module):
                def __init__(self, f, s=0):
                    ...
                    self.static_predictor = nn.Linear(256, s) if s > 0 else None  # 支持约束
                def forward(self, x, s=None):
                    ...
                    return rec, rec.mean(1), z, None, pred_s
            ...
            class MLPAE(nn.Module):
                ...
                def forward(self, x, s=None):
                    ...
                    h = self.enc[:-1](flat)  # 到中间层
                    z = self.enc[-1](h)
                    rec = self.dec(z).reshape(x.size(0), MIN_SEQ_LEN, -1)
                    pred_s = self.static_predictor(h) if self.static_predictor else None
                    return rec, rec.mean(1), z, None, pred_s
        ...
        for epoch in range(40):
            ...
            for batch in current_loader:
                if isinstance(batch, (list, tuple)):
                    ...
                    x_static = None
                rec_seq, rec_global, z, _, pred_static = model_ab(x_dyn, x_static)
