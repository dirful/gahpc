...
def main():
    ...
    if has_cpi:
        ...
        cpi = np.maximum(sequences[:, :, idx], 0)  # 防止负值

        # 第一斧：log1p 压右尾
        cpi_log = np.log1p(cpi)

        # 第二斧：开平方，进一步压缩右尾（比 log 更温和，但对极端值压制强）
        cpi_sqrt = np.sqrt(cpi_log)

        # 第三斧：标准化 + 强制对称 clip + 轻微左移（让分布更居中）
        cpi_standard = StandardScaler().fit_transform(cpi_sqrt.reshape(-1, 1)).reshape(cpi.shape)
        cpi_clipped = np.clip(cpi_standard, -3.0, 3.0)

        # 可选：如果仍偏右，可以整体左移一点（让均值更靠近0）
        cpi_final = cpi_clipped - 0.3  # 轻微左移 0.3（根据实际打印调整 0.2~0.5）
        cpi_final = np.clip(cpi_final, -3.0, 3.0)  # 再次 clip 防止下溢

        sequences[:, :, idx] = cpi_final

        # === 额外检查：打印 CPI 处理后的详细分布 ===
        cpi_after = sequences[:, :, idx].flatten()
        print("\n【CPI 处理后分布检查】")
        print(f"   mean(均值)    : {cpi_after.mean():.4f}")
        print(f"   std(标准差)   : {cpi_after.std():.4f}")
        print(f"   min / max     : {cpi_after.min():.4f} / {cpi_after.max():.4f}")
        print(f"   median(中位数): {np.median(cpi_after):.4f}")
        print(f"   5%  分位数    : {np.percentile(cpi_after, 5):.4f}")
        print(f"   95% 分位数    : {np.percentile(cpi_after, 95):.4f}")
        print(f"   99% 分位数    : {np.percentile(cpi_after, 99):.4f}")
        print("-" * 50)

        print("CPI 加强对齐处理完成：log1p → sqrt → StandardScaler → clip(-3,3) → 轻微左移")
        print(f"   CPI 处理后统计: mean={cpi_final.mean():.4f}, std={cpi_final.std():.4f}, "
              f"min={cpi_final.min():.4f}, max={cpi_final.max():.4f}, median={np.median(cpi_final):.4f}")
    # 全局标准化动态特征
    ...
    X_np = np.clip(X_np, -3.5, 3.5)  # 稍松一点，防止过度截断
    ...
    for i, name in enumerate(feature_names):
        ...
        print(f"{name:25}: mean={data2.mean():.4f}, std={data2.std():.4f}, min={data2.min():.4f}, max={data2.max():.4f}")
