...
from utils.LearnableFeatureWeights import LearnableFeatureWeights
...
# ✅ 新增：可学习权重模块
weight_module = LearnableFeatureWeights(WEIGHTS).to(DEVICE)
...
def main():
    ...
    def run_ablation_variant(name, main_loader=None, custom_loader=None, weights=WEIGHTS, model_class=WeightedTransAE):
        ...
        # 只有 Transformer + 使用加权损失时，才学习权重
        if isinstance(model_ab, WeightedTransAE) and weights is not None and weight_module is not None:
