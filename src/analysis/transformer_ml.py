...
# ========================== 核心：业务感知加权损失（你论文的灵魂）==========================
# ========================== 完全可学习的特征权重（方案B） ==========================
# 不再使用手工权重，所有10维特征权重都可学习
# base_weights 设为全1.0，learnable_mask 全1
weight_module = ConstrainedWeightModule(
    base_weights=torch.ones(10, device=DEVICE, dtype=torch.float32),  # 10维特征
    learnable_mask=torch.ones(10, device=DEVICE, dtype=torch.float32),  # 全可学习
    min_weight=0.1,    # 防止权重塌缩到0
    max_weight=3.0     # 防止权重爆炸
).to(DEVICE)

# 正向业务特征（越高越重要）
POS_IDX = [0, 1, 3, 6, 7, 8]  # cpu, mem, max_cpu, priority, cpu_req, mem_req

# 负向业务特征（越低越好）
NEG_IDX = [2, 5, 9]          # disk_io, CPI, disk_req

# 为了方便后面打印和解释，我们也定义一个特征名列表（保持和原来一致）
feature_names = [
...
def main():
    ...
    # 模型训练（完全可学习权重）
    model.train()
    alpha = 0.7  # 建议稍微提高一点，让时序重建更有主导权（可调0.65~0.85）

    for epoch in range(25):
        # 前10个epoch先冻结权重模块，让模型先学会基本重建
        if epoch < 10:
        ...
        for batch in loader:
            ...
            # 2. 业务加权全局重建损失（现在用可学习权重）
            target_global = x.mean(dim=1)  # [B, F]
            learned_weights = weight_module()  # [F]，自动输出当前学到的权重
            loss_weighted = (criterion_mse(rec_global, target_global) * learned_weights).mean()

            # 3. 总损失 + 正则项（防止权重极端）
            loss = alpha * loss_seq + (1 - alpha) * loss_weighted \
                   + 1e-4 * z.abs().mean() \
                   + 1e-3 * (weight_module.delta ** 2).mean()  # 鼓励权重平滑
        ...
        if (epoch + 1) % 5 == 0:
            ...
            print(f"Epoch {epoch+1:02d} | Loss: {total_loss/len(loader):.6f} | "
                  f"Learned Weights: {np.round(w, 3)}")
    # 提取表征 + 聚类 + 可视化（保持不变）
    model.eval()
    latents = []
    with torch.no_grad():
        ...
        for ep in range(25):
            ...
            for b in loader:
                ...
                if use_weighted:
                    ...
                    loss_weighted = (criterion_mse(rec_global, target_global) * weight_module()).mean()
    ...
    def run_ablation_variant(name, main_loader=None, custom_loader=None, weights=weight_module(), model_class=WeightedTransAE):
        print(f"\n>>> 正在训练: {name}")
        ...
        optimizer = optim.AdamW(list(model_ab.parameters()) + list(weight_module.parameters()),
            ...
            for batch in current_loader:
                ...
                if weights is not None:
                    if isinstance(weights, torch.Tensor) and weights.numel() == x.size(2):
                        w_use = weights.detach()  # 固定权重，直接 detach
                    else:
        ...
        if isinstance(model_ab, WeightedTransAE):
            ...
            with torch.no_grad():
                for batch in current_loader:
                    ...
                    for layer in model_ab.transformer.layers:
                        attn_out, attn_w = layer.self_attn(enc, enc, enc, need_weights=True, average_attn_weights=False)
            ...
            tc_score = temporal_consistency_score_v3(attn_time, labs,POS_IDX,NEG_IDX)
        else:
            tc_score = np.nan
    ...
    model_full, labels_full = run_ablation_variant("Ours (Full)", main_loader=loader, weights=weight_module())
    ...
    with torch.no_grad():
        for x in loader:
            ...
            # 2️⃣ 业务加权（注意裁剪维度）
            w_use = weight_module()[:recon_error.size(1)]
    ...
    business_weights = weight_module().detach().cpu().numpy()[:len(feature_names)]
